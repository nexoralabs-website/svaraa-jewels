<nav x-data="navbar()"
    ...
    <div class="container mx-auto px-4">
        <div class="flex items-center justify-between">
            <!-- Logo -->
            <a href="<?php echo e(route('home')); ?>" class="text-2xl font-bold text-[#6E0F12]" style="font-family: 'Playfair Display', serif;">
                Svaraa Jewels
            </a>

            <!-- Desktop Navigation -->
            <div class="hidden md:flex items-center space-x-8">
                <a href="<?php echo e(route('home')); ?>" class="nav-link text-[#2E1A12] hover:text-[#6E0F12] font-medium">Home</a>
                <a href="<?php echo e(route('products.index')); ?>" class="nav-link text-[#2E1A12] hover:text-[#6E0F12] font-medium">Earrings</a>
                <a href="<?php echo e(route('about')); ?>" class="nav-link text-[#2E1A12] hover:text-[#6E0F12] font-medium">About</a>
                <a href="<?php echo e(route('contact')); ?>" class="nav-link text-[#2E1A12] hover:text-[#6E0F12] font-medium">Contact</a>
            </div>
            ...
            <div class="hidden md:flex items-center space-x-6">
                ...
                <div class="relative" @click.outside="closeSearch">
                    <form action="<?php echo e(route('search.index')); ?>" method="GET" @submit="handleSubmit" class="relative">
                        <input type="text" 
                               name="q"
                               x-model="searchQuery"
                        ...
                        <template x-for="suggestion in suggestions" :key="suggestion.id">
                            <a :href="suggestion.url" 
                               class="flex items-center gap-3 px-4 py-3 hover:bg-[#F5EBDD] transition-colors first:rounded-t-xl last:rounded-b-xl border-b border-[#E8DCCB] last:border-0">
                                <img :src="suggestion.image" :alt="suggestion.name" class="w-12 h-12 rounded-lg object-cover flex-shrink-0 bg-gray-100" onerror="this.src='<?php echo e(asset('images/placeholder.jpg')); ?>'">
                ...
                <button @click="$dispatch('open-wishlist')" class="icon-action text-[#2E1A12] hover:text-[#6E0F12] relative">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(($wishlistCount ?? 0) > 0): ?>
                        <span class="absolute -top-2 -right-2 bg-[#C8A35D] text-[#2E1A12] text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center"><?php echo e($wishlistCount); ?></span>
        ...
        <div x-show="isOpen" 
            ...
            <div class="flex flex-col space-y-4 px-4">
                <a href="<?php echo e(route('home')); ?>" @click="isOpen = false" class="text-[#2E1A12] hover:text-[#6E0F12] transition-colors font-medium text-lg">Home</a>
                <a href="<?php echo e(route('products.index')); ?>" @click="isOpen = false" class="text-[#2E1A12] hover:text-[#6E0F12] transition-colors font-medium text-lg">Earrings</a>
                <a href="<?php echo e(route('about')); ?>" @click="isOpen = false" class="text-[#2E1A12] hover:text-[#6E0F12] transition-colors font-medium text-lg">About</a>
                <a href="<?php echo e(route('contact')); ?>" @click="isOpen = false" class="text-[#2E1A12] hover:text-[#6E0F12] transition-colors font-medium text-lg">Contact</a>
...
function navbar() {
    return {
        ...
        async fetchSuggestions() {
            ...
            try {
                const response = await fetch(`<?php echo e(route('search.suggest')); ?>?q=${encodeURIComponent(this.searchQuery)}`);
