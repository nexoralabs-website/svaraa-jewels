    ...
    <div class="bg-[#FFFFF0] min-h-[70vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-xl w-full bg-white p-10 text-center shadow-xl border border-[#C8A35D]/20 relative overflow-hidden">
            ...
            <h1 class="text-4xl font-serif text-[#6E0F12] mb-4">
                {{ $order->payment_method === 'cod' || $order->payment_status === 'captured' ? 'Thank You!' : 'Payment Processing' }}
            </h1>
            ...
            <div class="bg-gray-50 p-6 rounded-lg border border-gray-100 mb-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-left">
                ...
                <div class="sm:text-right">
                    ...
                    <p class="font-medium text-[#C8A35D]">{{ ucfirst($order->payment_status) }}</p>
                </div>
            </div>
            
            <div class="flex flex-col sm:flex-row justify-center gap-4">
                @auth
                <a href="{{ route('orders.show', $order) }}" class="inline-block bg-[#C8A35D] text-white px-8 py-3.5 uppercase tracking-widest text-sm font-medium hover:bg-[#b8934d] transition-colors">
                    View Order
                </a>
                @if(in_array($order->payment_status, ['captured', 'refunded']))
                <a href="{{ route('orders.invoice', $order) }}" class="inline-block border border-[#2E1A12] text-[#2E1A12] px-6 py-3.5 uppercase tracking-widest text-sm font-medium hover:bg-[#2E1A12] hover:text-white transition-colors flex items-center gap-2">
                ...
                @endif
                @endauth
                <a href="{{ route('products.index') }}" class="inline-block border border-[#6E0F12] text-[#6E0F12] px-8 py-3.5 uppercase tracking-widest text-sm font-medium hover:bg-[#6E0F12] hover:text-white transition-colors">
                    Continue Shopping
                </a>
            </div>
        </div>
