    ...
    <a href="{{ route('products.show', $product->slug) }}">
        <div class="relative overflow-hidden rounded-t-xl">
            <div class="aspect-square bg-[#E8DCCB] flex items-center justify-center product-image-container">
                <img src="{{ $product->thumbnail_url }}" alt="{{ $product->name }}" class="w-full h-full object-cover transition-all duration-500 group-hover:scale-110 group-hover:rotate-1" />
            </div>
        ...
        <div class="p-5">
            <p class="text-sm text-[#7B6755] mb-1">{{ $product->category?->name }}</p>
            <h3 class="text-lg font-semibold text-[#2E1A12] mb-2" style="font-family: 'Playfair Display', serif;">{{ $product->name }}</h3>
            <div class="flex items-center justify-between">
                <span class="text-xl font-bold text-[#6E0F12]">₹{{ number_format($product->price, 0) }}</span>
                <span class="text-xs text-[#7B6755]">{{ $product->purity }}</span>
            </div>
            <div class="product-cta mt-4 opacity-0 transform translate-y-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">
                <a href="{{ route('products.show', $product->slug) }}" class="block text-center rounded-full bg-[#6E0F12] px-4 py-2 text-sm font-semibold text-white hover:bg-[#4f0b0e] transition-colors">
                    View Product
                </a>
            </div>
        </div>
