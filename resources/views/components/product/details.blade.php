...
<div class="space-y-8" x-data="{ quantity: 1, inWishlist: false }">
    {{-- Header --}}
    <div>
        <p class="text-sm tracking-widest text-[#C8A35D] uppercase mb-2">{{ $product->category->name ?? 'Jewelry' }}</p>
        <h1 class="text-3xl md:text-4xl font-serif text-gray-900 mb-4">{{ $product->name }}</h1>
        <p class="text-2xl text-[#6E0F12] font-medium">₹{{ number_format($product->price, 2) }}</p>
    </div>
    ...
    <div class="border-t border-b border-gray-100 py-6">
        ...
        <dl class="grid grid-cols-2 gap-x-4 gap-y-4 text-sm">
            ...
            <div>
                ...
                <dd class="font-medium text-gray-900 mt-1">{{ $product->purity }}</dd>
            </div>
            @endif
            ...
            <div>
                ...
                <dd class="font-medium text-gray-900 mt-1">{{ $product->weight }}g</dd>
            </div>
            @endif
            
            <div>
                <dt class="text-gray-500">Availability</dt>
                <dd class="font-medium {{ $product->stock > 0 ? 'text-green-600' : 'text-red-500' }} mt-1">
    ...
    <div class="space-y-6" x-show="true" x-cloak>
        ...
        <div>
            ...
            <div class="flex items-center border border-gray-200 w-max bg-white">
                ...
                <input type="number" x-model="quantity" min="1" max="{{ $product->stock }}" class="w-16 text-center border-x border-gray-200 py-2 focus:ring-0 outline-none -moz-appearance: textfield;">
                <button type="button" @click="if(quantity < {{ $product->stock }}) quantity++" class="px-4 py-2 text-gray-500 hover:text-[#6E0F12] hover:bg-gray-50 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
        ...
        <div class="flex flex-col sm:flex-row gap-4 pt-4">
            <button type="button" @click="$store.cart.add({{ $product->id }}, quantity)" class="flex-1 bg-[#6E0F12] text-white px-8 py-4 uppercase tracking-widest text-sm font-medium hover:bg-[#520b0d] transition-colors flex justify-center items-center gap-2" {{ $product->stock <= 0 ? 'disabled' : '' }}>
