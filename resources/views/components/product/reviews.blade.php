@props(['product', 'reviews', 'distribution', 'canReview' => false, 'hasReviewed' => false])
    ...
    <div class="max-w-4xl">
        ...
        <div class="flex flex-col sm:flex-row gap-8 mb-10 p-6 bg-[#FDFBF7] rounded-xl border border-[#E8DCCB]/50">
            ...
            <div class="text-center sm:border-r sm:border-[#E8DCCB] sm:pr-8 flex-shrink-0">
                <p class="text-5xl font-serif text-[#2E1A12] leading-none">{{ number_format($product->average_rating, 1) }}</p>
                <div class="flex justify-center gap-0.5 mt-2" aria-label="{{ number_format($product->average_rating, 1) }} out of 5 stars">
                    @for($s = 1; $s <= 5; $s++)
                    <svg class="w-5 h-5 {{ $s <= round($product->average_rating) ? 'text-[#C8A35D]' : 'text-gray-200' }}"
                         fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                    @endfor
                </div>
                <p class="text-xs text-gray-500 mt-1">{{ $product->review_count }} {{ Str::plural('review', $product->review_count) }}</p>
            </div>
            ...
            <div class="flex-1 space-y-2">
                ...
                <div class="flex items-center gap-3">
                    <span class="text-xs text-gray-500 w-8 text-right flex-shrink-0">{{ $star }}★</span>
                    ...
                    <span class="text-xs text-gray-400 w-8 flex-shrink-0">{{ $count }}</span>
                </div>
                @endfor
            </div>
        </div>
        ...
        @auth
                    ...
                    <div>
                        ...
                        <div class="flex gap-1" role="radiogroup" aria-label="Rating">
                            ...
                            <button type="button"
                                    ...
                                    :aria-label="'{{ $s }} star'"
                                    :class="(hovered || rating) >= {{ $s }} ? 'text-[#C8A35D]' : 'text-gray-300'"
            ...
            <div class="mb-8 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
                ...
                <a href="{{ route('products.show', $product->slug) }}#" class="ml-1 font-medium underline">Shop now</a>
            </div>
            @endif
        @else
        <div class="mb-8 p-4 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-600">
            <a href="{{ route('login') }}" class="text-[#C8A35D] font-medium hover:underline">Log in</a> to write a review.
        </div>
        @endauth
            ...
            <div class="border-b border-gray-100 pb-6 last:border-0">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-medium text-[#2E1A12]">{{ $review->user->name ?? 'Anonymous' }}</p>
                    ...
                    <div class="flex flex-col items-end gap-1 flex-shrink-0">
                        <div class="flex gap-0.5" aria-label="{{ $review->rating }} out of 5 stars">
                            @for($s = 1; $s <= 5; $s++)
                            <svg class="w-4 h-4 {{ $s <= $review->rating ? 'text-[#C8A35D]' : 'text-gray-200' }}"
                                 fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>
                            @endfor
                ...
                @if($review->title)
                <h4 class="mt-3 text-sm font-semibold text-[#2E1A12]">{{ $review->title }}</h4>
                @endif
                @if($review->review)
                <p class="mt-2 text-sm text-gray-600 leading-relaxed">{{ $review->review }}</p>
                @endif
            </div>
            @endforeach
        </div>
