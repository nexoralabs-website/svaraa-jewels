<nav x-data="{ mobileOpen: false, cartOpen: false, searchOpen: false }" class="bg-white border-b border-gray-100 sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="hidden md:flex items-center space-x-10">
                <a href="{{ route('products.index') }}" class="text-sm font-medium text-gray-700 hover:text-[#6E0F12] transition-colors">
                    Earrings
                </a>
            </div>
            ...
            <div class="flex items-center gap-1 sm:gap-2">
                ...
                {{-- Wishlist --}}
                <a href="{{ route('wishlist.index') }}" class="p-2 text-gray-500 hover:text-[#6E0F12] transition-colors relative">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($wishlistCount > 0)
                    <span class="absolute -top-0.5 -right-0.5 w-4 h-4 bg-[#C8A35D] text-white text-[10px] font-bold rounded-full flex items-center justify-center">{{ $wishlistCount }}</span>
                    @endif
                </a>
                ...
                <div class="hidden sm:block ml-2 pl-2 border-l border-gray-200">
                    <a href="{{ route('dashboard') }}" class="text-sm font-medium text-gray-700 hover:text-[#6E0F12] transition-colors">
                        Account
                    </a>
                </div>
                @else
                <a href="{{ route('login') }}" class="hidden sm:block text-sm font-medium text-gray-700 hover:text-[#6E0F12] transition-colors ml-2 pl-2 border-l border-gray-200">
                    Sign In
                </a>
                @endauth
    ...
    <div :class="{'block': mobileOpen, 'hidden': !mobileOpen}" class="hidden md:hidden border-t border-gray-100 bg-white">
        <div class="px-4 pt-2 pb-4 space-y-2">
            <a href="{{ route('products.index') }}" class="block px-3 py-2 text-sm font-medium text-gray-700 hover:text-[#6E0F12] hover:bg-gray-50 rounded-md">Earrings</a>
        </div>
    </div>
